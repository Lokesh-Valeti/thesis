...
*/
void MinHeap1::insert_optimized(MPCTIO &tio, yield_t & yield, RegAS val) {
...
void MinHeap1::insert(MPCTIO &tio, yield_t & yield, RegAS val) {
...
void MinHeap1::verify_heap_property(MPCTIO &tio, yield_t & yield) {
...
*/
RegXS MinHeap1::restore_heap_property(MPCTIO &tio, yield_t & yield, RegXS index) {
    RegAS smallest;
    auto HeapArray = oram.flat(tio, yield);
...
std::pair<RegXS, RegBS> MinHeap1::restore_heap_property_optimized(MPCTIO &tio, yield_t & yield, RegXS index, size_t layer, typename Duoram < RegAS > ::template OblivIndex < RegXS, 3 > oidx) {
...
void MinHeap1::init(MPCTIO &tio, yield_t & yield) {
    auto HeapArray = oram.flat(tio, yield);
...
void MinHeap1::init(MPCTIO &tio, yield_t &yield, size_t n) {
    auto HeapArray = oram.flat(tio, yield);  // Get a flat representation of the ORAM.

    num_items = n;  // Store the number of elements in the heap.

    // Initialize all elements with zero
    HeapArray.init([](size_t i) {
...
// It is important to note that this function is not meant for production use and should be used solely for debugging purposes.
void MinHeap1::print_heap(MPCTIO &tio, yield_t & yield) {
    auto HeapArray = oram.flat(tio, yield);
...
*/
std::pair<RegXS, RegBS> MinHeap1::restore_heap_property_at_explicit_index(MPCTIO &tio, yield_t & yield, size_t index = 1) {
    auto HeapArray = oram.flat(tio, yield);
...
RegAS MinHeap1::extract_min(MPCTIO &tio, yield_t & yield, int is_optimized) {
...
int preIndex[10];
int letterToIndex(char x, int pos){
    char ch = x-'a'+1;
    if(pos==0){
        preIndex[pos]=ch;
        return ch;
        ...
        preIndex[pos] = preIndex[pos-1]*4+ch;
        return preIndex[pos];
    }
}
...
void Trie(unsigned player,MPCIO & mpcio,  const PRACOptions & opts, char ** args) {
    ...
    run_coroutines(tio, [ & tio, maxdepth, heapdepth, n_inserts, n_extracts, is_optimized, run_sanity,player, &mpcio](yield_t & yield) {
        size_t size = size_t(1) << maxdepth;
        MinHeap1 tree(tio.player(), size);
